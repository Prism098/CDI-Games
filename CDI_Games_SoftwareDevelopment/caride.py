...
#only relevant if not in full screen 
...
last_checkpoint = None  # Track the last checkpoint collected
...
# Starting position of the car ide background dropzone
grid_start_x, grid_start_y = (60,400)

# Drop Zone
DROP_ZONE_PADDING = 102
drop_zones = [pygame.Rect(DROP_ZONE_PADDING, 250 + i * 70, 250, 50) for i in range(MAX_STEPS)]
# move image down to add timer on screen 
image_top_padding = 160
image_right_padding = -600

# Grid alignment
GRID_PADDING_RIGHT = -200
...
# grid = (
#     ("R", "R", "C", "G", "D"),  # Finish line at (0, 4)
#     ("G", "C", "R", "R", "R"),
#     ("R", "R", "G", "R", "R"),  # Checkpoints at C, strategic placement of grass
#     ("R", "G", "G", "R", "R"),
#     ("R", "C", "R", "G", "R"),  # Start point at (4, 2)
# )
...
# Load images
background = pygame.image.load("CDI_Games_SoftwareDevelopment\\images\\Software_Dev\\caride_dropzone.png")
turn_left_img = pygame.image.load("CDI_Games_SoftwareDevelopment\\images\\Software_Dev\\caride_turn_left.png")
move_forward_img = pygame.image.load("CDI_Games_SoftwareDevelopment\\images\\Software_Dev\\caride_Forward.png")
turn_right_img = pygame.image.load("CDI_Games_SoftwareDevelopment\\images\\Software_Dev\\caride_turn_right.png")
run_button_img = pygame.image.load("CDI_Games_SoftwareDevelopment\\images\\Software_Dev\Run_button.png")
memory_full_img = pygame.image.load("CDI_Games_SoftwareDevelopment\\images\\Car_Memory_FULL_HIT_RUN_!.png")

# Load and scale maze image
CAR_IMAGE = pygame.image.load("CDI_Games_SoftwareDevelopment\\images\\Car_Green_Front.svg")
...
ROAD_IMAGE = pygame.image.load("CDI_Games_SoftwareDevelopment\\images\\weg_gravel.png")
...
background = pygame.transform.scale(background, (561, 1042))
...
def draw_timer(screen):
    ...
    screen.blit(timer_text, (timer_position))  # Top-left corner
    return time_left
...
def move_forward():
    global player_x, player_y, score, is_animation_complete, collected_checkpoints  # Include collected_checkpoints
...
def main():
    ...
    # Initialize the timer after the player starts the game
    global timer_start
    timer_start = time.time()
    ...
    while running:
        ...
        if memory_full:
            popup_x = GRID_OFFSET_X + (GRID_SIZE * TILE_SIZE - memory_full_img.get_width()) // 2
            popup_y = GRID_OFFSET_Y + (GRID_SIZE * TILE_SIZE - memory_full_img.get_height()) -600
